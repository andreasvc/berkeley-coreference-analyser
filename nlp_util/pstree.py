...
class TreeIterator:
	...
	PSTree uses pre-order traversal by default,
	but this supports post-order too, e.g.:
	>>> tree = tree_from_text(
...
class PSTree:
	...
	def check_consistency(self):
		...
		if len(self.subtrees) > 0:
			...
			if self.span != (self.subtrees[0].span[0],
					self.subtrees[-1].span[1]):
	...
	def get_nodes(self, request='all', start=-1, end=-1, node_list=None):
		...
		if request == 'lowest' and start < 0 and end < 0:
			raise Exception(
					"Lowest is not well defined when both ends are wildcards")
