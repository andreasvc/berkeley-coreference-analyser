...
def confusion_groups(gold_mentions, auto_mentions, gold_clusters,
		auto_clusters):
	groups = []
	mentions = set()
...
def mention_type(mention, text, parses, heads, lang):
	...
	if lang == 'en':
		if (mention[2] - mention[1] == 1
				and (head_pos
					in ["PRP", "PRP$", "WP", "WP$", "WDT", "WRB", "DT"]
					or head_word.lower() in english_pronoun_properties)):
			return "pronoun"
...
english_pronoun_properties = {
		...
		"they": (PRO_UNKNOWN, PRO_UNKNOWN, PRO_THIRD),  # NB technically plural
